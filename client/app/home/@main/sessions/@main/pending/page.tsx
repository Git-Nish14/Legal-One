...
import { GET_PENDING_SESSIONS } from "@/graphql/queries";
...
export default function PendingSessionsPage() {
  ...
  if (!data || data.getPendingSessions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-gray-700">
        ...
        <h2 className="text-xl font-semibold">No Pending Sessions</h2>
        <p className="text-gray-500 text-center max-w-sm mt-2">
          You currently have no Pending sessions. Once a session is scheduled, it will
          appear here.
        </p>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {data.getPendingSessions.map((session: any) => (
